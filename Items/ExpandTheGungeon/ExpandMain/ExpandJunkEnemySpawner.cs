        ...
        public void PlaceRandomJunkEnemies(Dungeon dungeon, RoomHandler roomHandler) {
            ...
            while (iterations < roomList.Count && RandomEnemiesPlaced < MaxEnemies) {
                ...
                try {
                    ...
                    if (1 == 9) ETGModConsole.Log("[DEBUG] Exception while setting up or placing enemy for current room" /*+ currentRoom.GetRoomName()*/, false);
                    if (1 == 9) ETGModConsole.Log("[DEBUG] Skipping current room...", false);
                    if (1 == 9) { ETGModConsole.Log(ex.Message + ex.StackTrace + ex.Source, false); }
                    if (RandomEnemiesPlaced >= MaxEnemies) { break; }
                    iterations++;
                }
            }
