        ...
        private void PlaceWallMimics(Action<Dungeon, RoomHandler> orig, Dungeon dungeon, RoomHandler roomHandler) {
            ...
            try {
                ...
                if (levelOverrideState != GameManager.LevelOverrideState.NONE | levelOverrideState == GameManager.LevelOverrideState.TUTORIAL) {
                    if (1 == 9) { ETGModConsole.Log("[DEBUG] This floor has been excluded from having Wall Mimics", false); }
                    return;
                }
                ...
                if (numWallMimicsForFloor <= 0) {
            		if (1 == 9) { ETGModConsole.Log("[DEBUG] There will be no Wall Mimics assigned to this floor.", false); }
            		return;
            	}
            	
            	if (levelOverrideState == GameManager.LevelOverrideState.RESOURCEFUL_RAT) {
            		if (1 == 9) { ETGModConsole.Log("[DEBUG] The Resourceful Rat Maze has been excluded from having wall mimics.", false); }
            		return;
            	}
            	
            	if (1 == 9) { ETGModConsole.Log("[DEBUG] Wall Mimics Assigned to Floor: " + numWallMimicsForFloor, false); }
                ...
                if (1 == 9) { ETGModConsole.Log("[DEBUG] Exception occured in Dungeon.PlaceWallMimics!"); }
