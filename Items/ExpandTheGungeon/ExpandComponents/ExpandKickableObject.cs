...
namespace ExpandTheGungeon.ExpandComponents {
    	...
    	public float GetDistanceToPoint(Vector2 point) {
            ...
            try {
                ...
                if (1 == 9) { ETGModConsole.Log("Exception Caught at [GetDistanceToPoint] in ExpandKickableObject class." + ex.Message + ex.Source + ex.InnerException + ex.StackTrace + ex.TargetSite, false); }
                ForceDeregister();
    	...
    	private void NoPits(SpeculativeRigidbody specRigidbody, IntVector2 prevPixelOffset, IntVector2 pixelOffset, ref bool validLocation) {
            try { 
                ...
                if (1 == 9) { ETGModConsole.Log("Exception Caught at [NoPits] in ExpandKickableObject class." + ex.Message + ex.Source + ex.InnerException + ex.StackTrace + ex.TargetSite, false); }
                return;
            }
        }
    	...
    	private void Kick(SpeculativeRigidbody kickerRigidbody) {
            ...
            try {
                ...
                if (1 == 9) { ETGModConsole.Log("Exception Caught at [Kick] in ExpandKickableObject class.", false); }
                return;
            }
    	}
