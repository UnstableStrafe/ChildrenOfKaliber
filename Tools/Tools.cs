    ...
    List<bool> fixesScales, List<Vector3?> manualOffsets, List<IntVector2?> overrideColliderPixelSizes, List<IntVector2?> overrideColliderOffsets, List<Projectile> overrideProjectilesToCopyFrom)
        {
            ...
            for (int i = 0; i < names.Count; i++)
            {
                ...
                def.ToolsConstructOffsetsFromAnchor(anchor, def.position3, fixesScale, anchorChangesCollider);
                def.ToolsMakeOffset(manualOffset.Value.XY(), true);
                if (i == 0)
                {
        ...
        public static void ToolsConstructOffsetsFromAnchor(this tk2dSpriteDefinition def, tk2dBaseSprite.Anchor anchor, Vector2? scale = null, bool fixesScale = false, bool changesCollider = true)
        {
            if (!scale.HasValue)
            {
            ...
            def.ToolsMakeOffset(new Vector2(xOffset, yOffset), changesCollider);
        }

        public static void ToolsMakeOffset(this tk2dSpriteDefinition def, Vector2 offset, bool changesCollider = false)
        {
